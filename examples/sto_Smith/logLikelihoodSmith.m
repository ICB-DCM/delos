function varargout = logLikelihoodSmith(theta, data, con0, timepoints, miniBatch)
...
for iMeasure = miniBatch
    ...
    if (nargout == 1)
        amiOptions.sensi = 0;
        sol = simulate_Smith(amiData.t, theta, amiData.condition, amiData, amiOptions);
        J = J - sol.llh;  
    elseif (nargout == 2)
        amiOptions.sensi = 1;
        sol = simulate_Smith(amiData.t, theta, amiData.condition, amiData, amiOptions);
...
varargout{1} = J * 100 / length(miniBatch);
if (nargout > 1)
    varargout{2} = gradJ * 100 / length(miniBatch);
end
